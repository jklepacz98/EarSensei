...
class GraphDataPreparer(val quizModels : ArrayList<QuizModel>, val xLabels : ArrayList<String>, val f : (QuizModel) -> String)  {
    fun prepareIntervalsHashMap() : LinkedHashMap<String, Float> {
    ...
    fun prepareIntervalsHashMap(numberOfEntries : Int) : LinkedHashMap<String, Float> {
        val ratioHashMap : LinkedHashMap<String, Float> = linkedMapOf()
        for(xLabel in xLabels){
            val filteredQuizModels : ArrayList<QuizModel> = ArrayList(quizModels
                .filter { f(it) == xLabel }
